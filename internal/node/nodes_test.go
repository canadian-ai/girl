...
func TestNodeTypes(t *testing.T) {
	t.Run("VariableNode", func(t *testing.T) {
		...
		n.IsConst = true
		if n.Kind() != KindVariable { t.Fatal("wrong kind") }
		if n.Name() != "userName" { t.Fatal("wrong name") }
		if n.DeclaredType != "string" { t.Fatal("wrong type") }
		if !n.IsConst { t.Fatal("expected const") }
	})
	...
	t.Run("ArrayNode", func(t *testing.T) {
		...
		if n.Kind() != KindArray { t.Fatal("wrong kind") }
		if len(n.Elements) != 2 { t.Fatal("wrong element count") }
	})

	t.Run("ObjectNode", func(t *testing.T) {
		n := NewObjectNode("obj_1", "config")
		n.Properties["key"] = "val_1"
		if len(n.Properties) != 1 { t.Fatal("wrong prop count") }
		if n.Properties["key"] != "val_1" { t.Fatal("wrong prop") }
	})
	...
	t.Run("FunctionNode", func(t *testing.T) {
		...
		n.IsArrow = false
		n.Params = []NodeID{"p_1"}
		if n.Kind() != KindFunction { t.Fatal("wrong kind") }
		if !n.IsAsync { t.Fatal("expected async") }
	})
	...
	t.Run("ComponentNode", func(t *testing.T) {
		...
		if n.Kind() != KindComponent { t.Fatal("wrong kind") }
		if n.Lines != 120 { t.Fatal("wrong lines") }
		if !n.IsExport { t.Fatal("expected export") }
		if len(n.Hooks) != 2 { t.Fatal("wrong hook count") }
	})

	t.Run("HookNode", func(t *testing.T) {
		n := NewHookNode("hook_1", "useState")
		n.Deps = []NodeID{"d_1"}
		if n.Kind() != KindHook { t.Fatal("wrong kind") }
		if n.Name() != "useState" { t.Fatal("wrong name") }
	})
	...
	t.Run("StateNode", func(t *testing.T) {
		...
		n.Value = "val_1"
		n.Setter = "set_1"
		if n.Kind() != KindState { t.Fatal("wrong kind") }
	})
	...
	t.Run("JSXNode", func(t *testing.T) {
		...
		if n.Kind() != KindJSX { t.Fatal("wrong kind") }
		if n.ElementType != "div" { t.Fatal("wrong element") }
		if n.Depth != 3 { t.Fatal("wrong depth") }
	})

	t.Run("ReferenceNode", func(t *testing.T) {
		n := NewReferenceNode("ref_1", "userName", "var_1", UsageRead)
		if n.Kind() != KindReference { t.Fatal("wrong kind") }
		if n.Target != "var_1" { t.Fatal("wrong target") }
		if n.Usage != UsageRead { t.Fatal("wrong usage") }
	})

	t.Run("CallNode", func(t *testing.T) {
		n := NewCallNode("call_1", "useQuery")
		n.Arguments = []NodeID{"arg_1"}
		if n.Kind() != KindCall { t.Fatal("wrong kind") }
		if len(n.Arguments) != 1 { t.Fatal("wrong args") }
	})
	...
	t.Run("ImportNode", func(t *testing.T) {
		...
		if n.Kind() != KindImport { t.Fatal("wrong kind") }
		if n.Source != "react" { t.Fatal("wrong source") }
		if len(n.Named) != 2 { t.Fatal("wrong named count") }
	})

	t.Run("ExportNode", func(t *testing.T) {
		n := NewExportNode("exp_1", "UserForm")
		n.IsDefault = true
		if n.Kind() != KindExport { t.Fatal("wrong kind") }
		if !n.IsDefault { t.Fatal("expected default") }
	})
	...
	t.Run("ConditionalNode", func(t *testing.T) {
		...
		if n.Kind() != KindConditional { t.Fatal("wrong kind") }
	})

	t.Run("EventNode", func(t *testing.T) {
		n := NewEventNode("evt_1", "handleClick")
		n.EventType = "onClick"
		if n.Kind() != KindEvent { t.Fatal("wrong kind") }
	})
}
