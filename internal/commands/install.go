...
//go:embed install-files/opencode install-files/claude install-files/codex install-files/pi
...
If no framework is specified, prints available frameworks.`,
		Action: func(c *cli.Context) error {
			...
			if framework == "" {
				fmt.Fprintln(os.Stderr, "Available frameworks: opencode, claude, codex, pi")
			...
			if !ok {
				return fmt.Errorf("unknown framework %q; supported: opencode, claude, codex, pi", framework)
			}
